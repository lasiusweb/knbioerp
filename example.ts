...
const orderConfig: IOrderConfig = {
  rfqThreshold: 10000, // Example threshold
  ordersEndpoint: '/orders',
  rfqEndpoint: '/rfq'
};

// Initialize OrderService
const orderService = new OrderService(client, orderConfig);

// Example: Prompt 3 Integration - Order Management
async function demonstratePrompt3() {
  try {
    console.log('=== Prompt 3: Order Management Integration ===');

    // Example order data
    const orderData = {
      status: 'pending' as const,
      total_amount: 5000, // Below threshold
      items: [
        {
          product_id: 'prod_123',
          quantity: 10,
    ...
    // Create order (should create direct order)
    const createdOrder = await orderService.createOrder(orderData);
    console.log('Created order:', createdOrder);

    // Example high-value order that triggers RFQ
    const highValueOrderData = {
      status: 'pending' as const,
      total_amount: 15000, // Above threshold
      items: [
        {
          product_id: 'prod_456',
          quantity: 5,
    ...
    // Create RFQ (should trigger RFQ logic)
    const rfqOrder = await orderService.createOrder(highValueOrderData);
    console.log('Created RFQ:', rfqOrder);

    // Fetch order history
    const orderHistory = await orderService.fetchOrderHistory('cust_123', 10);
    console.log('Order history:', orderHistory);

  } catch (error) {
    console.error('Prompt 3 demonstration failed:', error);
  }
}
